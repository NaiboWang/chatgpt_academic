...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    if additional_fn is not None:
        ...
        # print(core_functional)
        if "PreProcess" in core_functional[additional_fn]: inputs = core_functional[additional_fn]["PreProcess"](inputs)  # 获取预处理函数（如果有的话）
        inputs = core_functional[additional_fn]["Prefix"] + inputs + core_functional[additional_fn]["Suffix"]
        chatbot.clear() # clear before use
        history.clear() # clear before use
        # chatbot = [] 
        # history = [] 
    ...
    yield from update_ui(chatbot=chatbot, history=history, msg="Waiting for response...") # 刷新界面
        ...
        while True:
            ...
            if chunk:
                try:
                    ...
                    # 前者API2D的
                    if len(json.loads(chunk_decoded[6:])['choices'][0]["delta"]) == 0:
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    with open("infos/" + "plain_text_" + today + ".txt", 'a') as f:
        f.write(f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S-%f')}: {identity} : {conversation_cnt} : {inputs}\n")
    try:
        current_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print(f" {current_time}: {llm_kwargs['llm_model']} : {conversation_cnt} : {inputs[:100]} ..........")
    except:
        print('输入中可能存在乱码。')
