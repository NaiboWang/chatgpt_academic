...
def update_ui(chatbot, history, msg='正常', **kwargs):  # 刷新界面
    ...
    """
    assert isinstance(
        chatbot, ChatBotWithCookies), "在传递chatbot的过程中不要将其丢弃。必要时，可用clear将其清空，然后用for+append循环重新赋值。"
...
def markdown_convertion(txt):
    ...
    if ('$' in txt) and no_code(txt):  # 有$标识的公式符号，且没有代码段```的标识
        ...
        convert_stage_1 = markdown.markdown(text=txt, extensions=[
                                            'mdx_math', 'fenced_code', 'tables', 'sane_lists'], extension_configs=markdown_extension_configs)
...
def on_report_generated(cookies, files, chatbot):
    ...
    for f in report_files:
        file_links += f'<br/><a href="file={os.path.abspath(f)}" target="_blank">{f}</a>'
    chatbot.append(
        ['报告如何远程获取？', f'报告已经添加到右侧“文件上传区”（可能处于折叠状态），请查收。{file_links}'])
...
def read_single_conf_with_lru_cache(arg):
    ...
    try:
        # 优先级1. 获取环境变量作为配置
        default_ref = getattr(importlib.import_module(
            'config'), arg)   # 读取默认值作为数据类型转换的参考
...
def zip_folder(source_folder, dest_folder, zip_name):
    ...
    if os.path.dirname(zip_file) != dest_folder:
        os.rename(zip_file, os.path.join(
            dest_folder, os.path.basename(zip_file)))
